...
export const CONVERSATIONS: Conversation[] = [
  // ───────────────────────── DAY 1 · только Маю ─────────────────────────
  {
    ...
    steps: [
      ...
      think('«запости что-нибудь». а что люди вообще пишут? про еду?'),
    ],
  },
  {
    ...
    steps: [
      ...
      my('видела. лайк. ✌🏻'),
      think('один лайк. маю. ну считается же'),
    ],
  },
  {
    id: 'd1_mayu_no_post',
  ...
  // тихий первый след: чей-то лайк поздно ночью
  {
    ...
    at: t(23, 50),
    when: (s) => !!s.flags.posted,
    steps: [{ type: 'like', author: 'REN_17' }, { type: 'set', flags: ['ren_liked'] }, think('какой-то ren_17. лайкнул в полночь. ок')],
  },
  ...
  },
  {
    ...
    steps: [
      ...
      think('опять этот ren_17. пишет так, будто мы знакомы. мы не знакомы'),
    ],
  },
  {
    id: 'd2_mayu_late',
  ...
  },
  {
    ...
    steps: [
      on('ren'),
      pause(5000),
      think('тот с комментами онлайн. ну и что'),
      ...
      pause(1500),
      off('ren'),
      think('перечитала «значит нас двое» раза три. зачем, непонятно'),
    ],
  },
  ...
  },
  {
    ...
    steps: [
      ...
      think('фото с лицом у меня одно. с вебки, прошлогоднее. где-то в photos'),
      {
        type: 'choice',
        options: [
      ...
      { type: 'toast', title: 'Загрузки', text: 'track07.mp3 — загрузка завершена', icon: '/assets/icons/media-player.png', action: { app: 'music' } },
      r('послушай когда меня не будет'),
      off('ren'),
      think('track07. без названия даже. послушаю потом. нет, сейчас'),
    ],
  },
  ...
  },
  {
    ...
    steps: [
      ...
      think('«пользователь не найден». обновила. ещё раз обновила. ещё'),
      { type: 'wallpaper', value: 'night' },
    ],
  },
  ...
  },
  {
    ...
    steps: [
      ...
      pause(3000),
      sys('вот он.'),
      sys('ниже, чем на фото. волосы другие. очки. тыкает в телефон и всё время поправляет куртку.'),
      sys('не тот, кого ты себе придумала. совсем.'),
      sys('он поднимает голову. кажется, он думает то же самое.'),
      {
        type: 'choice',
        options: [
...
export const ENDINGS: Record<string, Ending> = {
  mayu: {
    title: 'MAYU',
    lines: [
      'окно закрыто. комп не выключен — некогда.',
      'на улице холодно и пахнет дождём. маю машет с другой стороны дороги термосом.',
      'единственная фотка с моря — размазанная, пересвеченная, горизонт завален. маю ржёт. нана почти улыбается.',
      'маю поставит её на аватарку. нана не будет против.',
    ],
  },
  stay_online: {
    title: 'STAY ONLINE',
    lines: ['04:13', 'nana — online', 'REN_17 — online', 'mayu☆ — offline', '', 'за окном сереет. в комнате светит только монитор.', 'термос так и стоит у двери.'],
  },
  user_not_found: {
    title: 'USER NOT FOUND',
    lines: ['страницы REN_17 больше нет.', 'ни объяснения, ни последнего сообщения. только 404.', 'нана ещё недели две открывает «сообщения» по привычке. там только маю.', 'track07 так и лежит в загрузках.'],
  },
  meet_me: {
    title: 'MEET ME',
    lines: [
      'он не похож на свои фото.',
      'ниже. тише. всё время поправляет очки.',
      'первые пять минут они просто стоят.',
      'потом он говорит: «ты тоже не похожа». и потом: «ну, пойдём?»',
      'идут за кофе. неловко. по настоящему.',
    ],
  },
  meet_me_left: {
    title: 'MEET ME',
    lines: [
      ...
      'в электричке перечитывает последнее: «серая куртка».',
      'не отвечает. он больше не пишет.',
      'через неделю она ловит себя на том, что скучает. не по нему — по окошку с его ником.',
    ],
  },
  log_off: {
    title: 'LOG OFF',
    lines: ['нана выключает комп.', 'не решила — просто устала.', 'маю не пишет неделю. потом присылает «✌». нана отвечает через день.', '«✌».'],
  },
  balance: {
    title: 'ONLINE / OFFLINE',
    lines: [
      'вечером нана пишет рэну, как обещала. коротко. и ложится.',
      'утром на рабочем столе одна новая фотка: море, пустой термос, маю с закрытыми глазами.',
      'REN_17 — offline.',
      'нормально. напишет позже.',
    ],
  },
};
