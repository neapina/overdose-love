...
/** play a real track file; resolves the element so the caller can read position */
export function playMusic(file: string, onEnded?: () => void): HTMLAudioElement {
  stopMusic();
  const m = new Audio(file);
  m.volume = 0;
  m.muted = getState().muted;
  m.preload = 'auto';
  m.onended = () => {
    if (music === m) onEnded?.();
  };
  ...
  let v = 0;
  const fade = setInterval(() => {
    if (music !== m) return clearInterval(fade);
